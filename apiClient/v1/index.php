...
Flight::route('GET /getCatalogs/@headerslink/@clientId/@filter/@param/@value/@st', function ($headerslink,$clientId,$filter,$param,$value,$st) {
    ...
    if (!empty($apiKey) && !empty($xApiKey)) {
      ...
      curl_close($curl);
        ...
        if ($response1 != 'false' ) {
            ...
            // Realizar la solicitud y obtener la respuesta
            $response = file_get_contents($sub_domain.'/kairosCom/apiClient/v1/getCatalogs/'.$clientId.'/'.$filter.'/'.$param.'/'.$value.'/'.$st, false, $context);
...
$status = http_response_code();
$cid=Flight::request()->data->clientId;
//kronos($response2,$response2,$response2, $info['Función'],$justFileName,$rutaCompleta,$cid,$dt,$url,$status);
//final de log
//echo $response2;
echo  kronos($response2,$response2,$response2, $info['Función'],$justFileName,$rutaCompleta,$cid,$dt,$url,$status);
